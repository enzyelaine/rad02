...
function prime_custom_navigation_setup() {
	...
	//Check for Upgrades
	if (get_option('prime_settings_custom_nav_version') <> '') {
		$nav_version_in_db = get_option('prime_settings_custom_nav_version');
	}
	else {
		$nav_version_in_db = '0';
	}
	...
	update_option('prime_settings_custom_nav_advanced_options','yes');
	...
	if ($page_var == 'custom_navigation') 
	{
		...
		if(($wpdb->get_var("show tables like '$table_name'") != $table_name) || ($nav_version_in_db <> $nav_version)) 
		{
			...
			update_option('prime_settings_custom_nav_version',$nav_version);
			...
			if ($data_insert) {
				...
				foreach ($pages_array as $post) 
				{
					...
					if ($post->post_parent == 0) 
					{
						...
						$table_name_parent = $wpdb->prefix . "prime_custom_nav_records";
						$prime_result = $wpdb->get_results("SELECT id FROM ".$table_name_parent." WHERE post_id='".$post->post_parent."' AND link_type='page' AND menu_id='1'");
				...
				foreach ($categories_array as $cat_item) {
					...
					if ($cat_item->parent == 0)
					{
						...
						$table_name_parent = $wpdb->prefix . "prime_custom_nav_records";
						$prime_result = $wpdb->get_results("SELECT id FROM ".$table_name_parent." WHERE post_id='".$cat_item->parent."' AND link_type='category' AND menu_id='2'");
...
function prime_custom_nav_reset() {
...
function prime_custom_navigation_menu() {

	//primethemes Custom Navigation Menu	
	$primepage = add_submenu_page('primethemes', 'Custom Navigation', 'Custom Navigation', 8, 'custom_navigation', 'prime_custom_navigation');
	
	add_action("admin_print_scripts-$primepage", 'prime_custom_nav_scripts' );
	  	
}

function prime_custom_nav_scripts() {
	...
	wp_register_script('prime-nav-dynamic', get_bloginfo('template_directory').'/functions/js/custom_menu_dynamic_items.js', array( 'jquery-ui-dialog' ));
	wp_enqueue_script('prime-nav-dynamic');
	wp_register_script('prime-nav-initial', get_bloginfo('template_directory').'/functions/js/custom_menu_initial_items.js', array( 'jquery-ui-dialog' ));
	wp_enqueue_script('prime-nav-initial');
	wp_register_script('prime-nav-autocomplete', get_bloginfo('template_directory').'/functions/js/jquery.autocomplete.js', array( 'jquery' ));
	wp_enqueue_script('prime-nav-autocomplete');
	//Default Style
...
function prime_custom_navigation() {
	...
	<div id="no-js"><h3>You do not have JavaScript enabled in your browser. Please enabled it to access the Custom Menu functionality.</h3></div>
	    ...
	    $table_name_menus = $wpdb->prefix . "prime_custom_nav_menus";	 	 
		$prime_result = $wpdb->get_results("SELECT id FROM ".$table_name_menus." ORDER BY id ASC LIMIT 1");
 		if ($prime_result > 0 && isset($prime_result[0]->id)) {
			$menu_selected_id = $prime_result[0]->id;
		}
		else {
			$menu_selected_id = 1;
		}
	    ...
	    if (isset($_POST['set_prime_menu']))
	    {
	    	update_option('prime_custom_nav_menu', $_POST['enable_prime_menu']);
	    	$messagesdiv = '<div id="message" class="updated fade below-h2"><p>'.$themename.'\'s Custom Menu has been updated!</p></div>';
	    } 
	  
	    	    
	    //CHECK for existing prime custom menu
	 	$table_name = $wpdb->prefix . "prime_custom_nav_records";	 	
		...
		if (isset($_POST['add_menu'])) {
			...
			$table_name_custom_menu = $wpdb->prefix . "prime_custom_nav_menus";
		...
		if ($postCounter > 0) 
		{
			...
			elseif (isset($_POST['reset_prime_menu'])) {	
	    		$success = prime_custom_nav_reset();
	    		if ($success) {
			...
			if (isset($_POST['reset_prime_menu'])) {	
	    		$success = prime_custom_nav_reset();
	    		if ($success) {
	    			//DISPLAY SUCCESS MESSAGE IF Menu Reset Correctly
 					$messagesdiv = '<div id="message" class="updated fade below-h2"><p>'.$themename.'\'s Custom Menu has been RESET!</p></div>';
		...
		<div id="pages-left">
			<div class="inside">
			<h2 class="maintitle"><img class="logo" src="<?php bloginfo('template_directory'); ?>/functions/images/logo.png" alt="primethemes" />Custom Navigation</h2>
			<?php
				
				//CHECK if custom menu has been enabled
				$enabled_menu = get_option('prime_custom_nav_menu');
					...
					<?php
						...
						if (($custom_nav_exists) > 0 || ($custom_menu_name > 0)) 
						{
							...
							if ($menu_id_in_edit > 0)
							{
								...
								prime_custom_navigation_output('type='.$output_type.'&name='.$menu_name.'&id='.$menu_id_in_edit);
							}
							else 
							{
								//MAIN OUTPUT FUNCTION
								prime_custom_navigation_output('type='.$output_type.'&name='.$menu_name.'&id='.$menu_selected_id);
							}
						...
						{	
							...
							$intCounter = prime_get_pages(1,'menu');			
							//Outputs default Categories
							$intCounter = prime_get_categories($intCounter,'menu');
						}				
					?>
		...
		<div id="menu-right">
			...
			<div class="widgets-holder-wrap">
				...
				<div class="widget-holder">
			    	...
			    	<span >
			    		<label>Enable</label><input type="radio" name="enable_prime_menu" value="true" <?php if ($checked=='true') { echo 'checked="checked"'; } ?> />
			    		<label>Disable</label><input type="radio" name="enable_prime_menu" value="false" <?php if ($checked=='true') { } else { echo 'checked="checked"'; } ?> />
					</span><!-- /.checkboxes -->				
						
					<input id="set_prime_menu" type="submit" value="Set Menu" name="set_prime_menu" class="button" /><br />
					
					<span>
						<label>Reset Menu to Default</label>
						<input id="reset_prime_menu" type="submit" value="Reset" name="reset_prime_menu" class="button" onclick="return confirm('Are you sure you want to RESET the Custom Navigation Menu to its Default Settings?');" />
					</span>
			...
			<div class="widgets-holder-wrap">
				...
				<div class="widget-holder">
					...
					$table_name_custom_menus = $wpdb->prefix . "prime_custom_nav_menus";
			...
			<?php $advanced_option_descriptions = get_option('prime_settings_custom_nav_advanced_options'); ?>
				...
				<div class="widget-holder">	
			    	...
			    	if (isset($_POST['menu-descriptions']))
			    	{
						...
						if (isset($_POST['switch_menu'])) {
			    			...
			    			update_option('prime_settings_custom_nav_'.$menu_options_to_edit.'_descriptions',$_POST['menu-descriptions']);	
						}
			    	...
			    	if ($menu_id_in_edit > 0)
					{
						$checkedraw = get_option('prime_settings_custom_nav_'.$menu_id_in_edit.'_descriptions');
					}
					else {
						$checkedraw = get_option('prime_settings_custom_nav_'.$menu_selected_id.'_descriptions');
					}
			...
			<div class="widgets-holder-wrap">
				...
				<div class="widget-holder">
					...
					if ($pages_array)
					{
						...
						{
							//Convert string to UTF-8
							$str_converted = prime_encoding_convert($post->post_title);
							//Add page name to 
					...
					<ul id="existing-pages" class="list">
						<?php
							$intCounter = 0;
							//Get default Pages
							$intCounter = prime_get_pages($intCounter,'default');
						?>
					</ul>
				...
				<div class="widget-holder">
					...
					if ($categories)
					{
						...
						{ 
							...
							{
								//Convert string to UTF-8
								$str_converted = prime_encoding_convert($category_names[0]->name);
					...
					<ul id="existing-categories" class="list">
            			<?php
            				...
            				$intCounter = prime_get_categories($intCounter,'default'); 				
						?>
       				</ul>
...
function prime_custom_navigation_output($args = array()) {
		...
		global $wpdb;
		$prime_custom_nav_menu_id = 0;
		$table_name = $wpdb->prefix . "prime_custom_nav_records";
		
		//Override for menu descriptions
		$advanced_option_descriptions = get_option('prime_settings_custom_nav_advanced_options');
		if ($advanced_option_descriptions == 'no')
		{
			$desc = 2;
		}
		...
		if ($type == "frontend") 
		{
			$table_name_menus = $wpdb->prefix . "prime_custom_nav_menus";
			if ($id > 0) {
				$prime_custom_nav_menu_id = $id;
			}
			else {
				$prime_result = $wpdb->get_results("SELECT id FROM ".$table_name_menus." WHERE menu_name='".$name."'");
				$prime_custom_nav_menu_id = $prime_result[0]->id;
			}
			
			$prime_custom_nav_menu = $wpdb->get_results("SELECT id,post_id,parent_id,position,custom_title,custom_link,custom_description,menu_icon,link_type,custom_anchor_title,new_window FROM ".$table_name." WHERE parent_id = '0' AND menu_id='".$prime_custom_nav_menu_id."' ORDER BY position ASC");
		}
		//BACKEND
		else {
			$prime_custom_nav_menu = $wpdb->get_results("SELECT id,post_id,parent_id,position,custom_title,custom_link,custom_description,menu_icon,link_type,custom_anchor_title,new_window FROM ".$table_name." WHERE parent_id = '0' AND menu_id='".$id."' ORDER BY position ASC");
		}
		$queried_id = 0;
		...
		foreach ($prime_custom_nav_menu as $prime_custom_nav_menu_items) {
			...
			if ($prime_custom_nav_menu_items->link_type == 'page')
			{
				if ($prime_custom_nav_menu_items->custom_link == '') {
					$link = get_permalink($prime_custom_nav_menu_items->post_id);
				}
				else {
					$link = $prime_custom_nav_menu_items->custom_link;
				}

				if ($prime_custom_nav_menu_items->custom_title == '') {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert(get_the_title($prime_custom_nav_menu_items->post_id));
					$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				}
				else {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert($prime_custom_nav_menu_items->custom_title);
					$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				}

				if ($prime_custom_nav_menu_items->custom_description == '') {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert(get_post_meta($prime_custom_nav_menu_items->post_id, 'page-description', true));
					$description = htmlspecialchars(trim($str_converted), ENT_QUOTES, 'UTF-8');
				}
				else {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert($prime_custom_nav_menu_items->custom_description);
					$description = htmlspecialchars(trim($str_converted), ENT_QUOTES, 'UTF-8');
				}
				$target = '';
			}
			//Category Menu Item
			elseif ($prime_custom_nav_menu_items->link_type == 'category') 
			{
				
				if ($prime_custom_nav_menu_items->custom_link == '') {
					$link = get_category_link($prime_custom_nav_menu_items->post_id);
				}
				else {
					$link = $prime_custom_nav_menu_items->custom_link;
				}
				...
				if ($prime_custom_nav_menu_items->custom_title == '') {
					$title_raw = get_categories('include='.$prime_custom_nav_menu_items->post_id);
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert($title_raw[0]->cat_name);
					$title =  htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');	
				}
				else {
					//Convert string to UTF-8
				...
				if ($prime_custom_nav_menu_items->custom_description == '') {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert(category_description($prime_custom_nav_menu_items->post_id));
			...
			//SET anchor title
			if (isset($prime_custom_nav_menu_items->custom_anchor_title)) {
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert($prime_custom_nav_menu_items->custom_anchor_title);
			...
			if (($queried_id == $prime_custom_nav_menu_items->post_id) && ($queried_id != 0) && ($type_settings == $prime_custom_nav_menu_items->link_type) ) {
				$li_class = 'class="current_page_item"';
			}
			else if (($prime_custom_nav_menu_items->custom_link == $full_web_address) && ($queried_id == 0) && ($type_settings == $prime_custom_nav_menu_items->link_type) ) {
				$li_class = 'class="current_page_item"';
			}
			else if (prime_child_is_current($prime_custom_nav_menu_items->id, $prime_custom_nav_menu_id, $table_name, $queried_id, $type_settings, $full_web_address)) {
			...
			if (isset($prime_custom_nav_menu_items->new_window)) {
				if ($prime_custom_nav_menu_items->new_window > 0) {
					$target = 'target="_blank"';
				}
				else {
					$target = '';
			...
			?><?php 
					...
					{
						?>
						<li id="menu-<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->position; ?>" <?php echo $li_class; ?>>
						<dl>
							<dt>
								<span class="title"><?php echo $title; ?></span>
								<span class="controls">
								<span class="type"><?php echo $prime_custom_nav_menu_items->link_type; ?></span>
								<a id="edit<?php echo $prime_custom_nav_menu_items->position; ?>" onclick="edititem(<?php echo $prime_custom_nav_menu_items->position; ?>)" value="<?php echo $prime_custom_nav_menu_items->position; ?>"><img class="edit" alt="Edit Menu Item" title="Edit Menu Item" src="<?php echo get_bloginfo('template_directory'); ?>/functions/images/ico-edit.png" /></a> 
								<a id="remove<?php echo $prime_custom_nav_menu_items->position; ?>" onclick="removeitem(<?php echo $prime_custom_nav_menu_items->position; ?>)" value="<?php echo $prime_custom_nav_menu_items->position; ?>"><img class="remove" alt="Remove from Custom Menu" title="Remove from Custom Menu" src="<?php echo get_bloginfo('template_directory'); ?>/functions/images/ico-close.png" /></a>
								<a id="view<?php echo $prime_custom_nav_menu_items->position; ?>" target="_blank" href="<?php echo $link; ?>"><img alt="View Page" title="View Page" src="<?php echo get_bloginfo('template_directory'); ?>/functions/images/ico-viewpage.png" /></a>
								</span>
							</dt>
						</dl>
						
						<a><span class=""></span></a>
						<input type="hidden" name="dbid<?php echo $prime_custom_nav_menu_items->position; ?>" id="dbid<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->id; ?>" />
						<input type="hidden" name="postmenu<?php echo $prime_custom_nav_menu_items->position; ?>" id="postmenu<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->post_id; ?>" />
						<input type="hidden" name="parent<?php echo $prime_custom_nav_menu_items->position; ?>" id="parent<?php echo $prime_custom_nav_menu_items->position; ?>" value="0" />
						<input type="hidden" name="title<?php echo $prime_custom_nav_menu_items->position; ?>" id="title<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $title; ?>" />
						<input type="hidden" name="linkurl<?php echo $prime_custom_nav_menu_items->position; ?>" id="linkurl<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $link; ?>" />
						<input type="hidden" name="description<?php echo $prime_custom_nav_menu_items->position; ?>" id="description<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $description; ?>" />
						<input type="hidden" name="icon<?php echo $prime_custom_nav_menu_items->position; ?>" id="icon<?php echo $prime_custom_nav_menu_items->position; ?>" value="0" />
						<input type="hidden" name="position<?php echo $prime_custom_nav_menu_items->position; ?>" id="position<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->position; ?>" />
						<input type="hidden" name="linktype<?php echo $prime_custom_nav_menu_items->position; ?>" id="linktype<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->link_type; ?>" />
						<input type="hidden" name="anchortitle<?php echo $prime_custom_nav_menu_items->position; ?>" id="anchortitle<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $anchor_title; ?>" />
						<input type="hidden" name="newwindow<?php echo $prime_custom_nav_menu_items->position; ?>" id="newwindow<?php echo $prime_custom_nav_menu_items->position; ?>" value="<?php echo $prime_custom_nav_menu_items->new_window; ?>" />
						
						<?php 
					}
					
					//DISPLAY menu sub items 
					if ($prime_custom_nav_menu_items->parent_id == 0) 
					{
						//FRONTEND
						if ($type == 'frontend') 
						{
							//Recursive function
							if ( ($depth == 0) || ($depth > 1) ) {
								$intj = prime_custom_navigation_sub_items($prime_custom_nav_menu_items->id,$prime_custom_nav_menu_items->link_type,$table_name,$type,$prime_custom_nav_menu_id, $depth, 1);
							}
						}
						//BACKEND
						else 
						{
							//Recursive function
							$intj = prime_custom_navigation_sub_items($prime_custom_nav_menu_items->id,$prime_custom_nav_menu_items->link_type,$table_name,$type,$id);
						}
					}
					else 
					{
...
function prime_custom_navigation_sub_items($post_id,$type,$table_name,$output_type,$menu_id = 0,$depth = 0,$depth_counter = 0) {
	...
	//GET sub menu items
	$prime_custom_nav_menu = $wpdb->get_results("SELECT id,post_id,parent_id,position,custom_title,custom_link,custom_description,menu_icon,link_type,custom_anchor_title,new_window FROM ".$table_name." WHERE parent_id = '".$post_id."' AND menu_id='".$menu_id."' ORDER BY position ASC");
	
	if (empty($prime_custom_nav_menu))
	{
	...
	else
	{
	    ...
	    //DISPLAY Loop
		foreach ($prime_custom_nav_menu as $sub_item) 
		{
			...
			if ($sub_item->link_type == 'category') 
			{
				...
				if ($sub_item->custom_title == '') {
					...
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert($sub_item->custom_title);
					$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				}
			...
			elseif ($sub_item->link_type == 'page')
			{
				...
				if ($sub_item->custom_title == '') {
					//Convert string to UTF-8
					$str_converted = prime_encoding_convert(get_the_title($sub_item->post_id));
					$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				}
				else {
					//Convert string to UTF-8
			...
			else if (prime_child_is_current($sub_item->id, $menu_id, $table_name, $queried_id, $type_settings, $full_web_address)) {
                $li_class = 'class="current_page_parent"';
		    }
			else {
				$li_class = '';
			...
			if (isset($sub_item->custom_anchor_title)) {
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert($sub_item->custom_anchor_title);
				$anchor_title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
			}
			else {
				$anchor_title = $title;
			...
			?><?php 
						...
						if ( ($depth_counter < $depth) || ($depth == 0) ) {
							prime_custom_navigation_sub_items($sub_item->id,$sub_item->link_type,$table_name,$output_type,$menu_id, $depth, $depth_counter);
						} 
			?></li>
			<?php 
...
function prime_child_is_current($parent_id, $menu_id, $table_name, $queried_id, $type_settings, $full_web_address) {
    ...
    //GET sub menu items
    $prime_parent_children = $wpdb->get_results("SELECT id,post_id,parent_id,position,custom_title,custom_link,custom_description,menu_icon,link_type,custom_anchor_title,new_window FROM ".$table_name." WHERE parent_id = '".$parent_id."' AND menu_id='".$menu_id."' ORDER BY position ASC");
	    
    //If more than 0 child elements
    if (empty($prime_parent_children))
    {
    ...
    {
       //Children Loop
        foreach ($prime_parent_children as $prime_parent_child) 
        {
            //Check if meets criteria
            if (($queried_id == $prime_parent_child->post_id) && ($queried_id != 0) && ($type_settings == $prime_parent_child->link_type) ) {
                $success = true;
            }
            else if (($prime_parent_child->custom_link == $full_web_address) && ($queried_id == 0) && ($type_settings == $prime_parent_child->link_type) ) {
                $success = true;
            }    
        }
    }
...
function prime_get_pages($counter,$type) {
			...
			if ($post->post_parent == 0)
			{
				...
				if ($type == 'menu')
				{
					...
					<li id="menu-<?php echo $intCounter; ?>" value="<?php echo $intCounter; ?>">
						...
						<?php $str_converted = prime_encoding_convert($post->post_title);?>
						...
						<?php
							...
							//Recursive function
							$intCounter = prime_custom_navigation_default_sub_items($post->ID, $intCounter, $parentli, 'pages', 'menu');
					...
					?>
					 ...
					 <li>
				        ...
				        <?php
				        	...
				        	//Convert string to UTF-8
							$str_converted = prime_encoding_convert(get_post_meta($post_id, 'page-description', true));
							$description = htmlspecialchars(trim($str_converted), ENT_QUOTES, 'UTF-8');
				        ...
				        <?php
							...
							//Recursive function
							$intCounter = prime_custom_navigation_default_sub_items($post_id, $intCounter, $parentli, 'pages', 'default');
...
function prime_get_categories($counter, $type) {
			...
			if ($cat_item->parent == 0)
			{
				...
				if ($type == 'menu')
				{
			    	...
			    	<li id="menu-<?php echo $intCounter; ?>" value="<?php echo $intCounter; ?>">
			            ...
			            <?php 
			            $use_cats_raw = get_option('prime_settings_custom_nav_descriptions');
			            ...
			            <input type="hidden" name="parent<?php echo $intCounter; ?>" id="parent<?php echo $intCounter; ?>" value="0" />
			            <?php $str_converted = prime_encoding_convert($cat_item->cat_name);?>
			            <input type="hidden" name="title<?php echo $intCounter; ?>" id="title<?php echo $intCounter; ?>" value="<?php echo htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8'); ?>" />
						...
						<?php $str_converted = prime_encoding_convert($cat_item->category_description);?>
			            ...
			            <?php $intCounter++; ?>			                
			           	<?php
							...
							//Recursive function
							$intCounter = prime_custom_navigation_default_sub_items($cat_item->cat_ID, $intCounter, $parentli, 'categories','menu');
			    ...
			    elseif ($type == 'default')
			    {
			    	...
			    	<li>
						...
						<?php
						//Convert string to UTF-8
						$str_converted = prime_encoding_convert($cat_item->cat_name);
	        			...
	        			//Convert string to UTF-8
						$str_converted = prime_encoding_convert($cat_item->description);
						...
						<?php 
							//Recursive function
							$intCounter = prime_custom_navigation_default_sub_items($cat_item->cat_ID, $intCounter, $parentli, 'categories','default');
						?>
...
function prime_custom_navigation_default_sub_items($childof, $intCounter, $parentli, $type, $output_type) {
	...
	if ($sub_array)
	{
		...
		foreach ($sub_array as $sub_item)
		{
			...
			if ($type == 'categories') 
			{
				...
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert($sub_item->cat_name);
				//$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				...
				$appendtype = 'Category';
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert($sub_item->description);
				//$description = htmlspecialchars(strip_tags(trim($str_converted)), ENT_QUOTES, 'UTF-8');	
				$description = htmlspecialchars(addslashes(strip_tags(trim($str_converted))), ENT_QUOTES, 'UTF-8');			
			}
			//Page Menu Item
			elseif ($type == 'pages')
			{
				...
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert($sub_item->post_title);
				//$title = htmlspecialchars($str_converted, ENT_QUOTES, 'UTF-8');
				$title = htmlspecialchars(addslashes($str_converted), ENT_QUOTES, 'UTF-8');
				...
				$appendtype = 'Page';
				//Convert string to UTF-8
				$str_converted = prime_encoding_convert(get_post_meta($itemid, 'page-description', true));
				//$description = htmlspecialchars(trim($str_converted), ENT_QUOTES, 'UTF-8');
				$description = htmlspecialchars(addslashes(strip_tags(trim($str_converted))), ENT_QUOTES, 'UTF-8');
			}
			//Custom Menu Item
			...
			if ($output_type == 'menu')
			{
				...
				<li id="menu-<?php echo $counter; ?>" value="<?php echo $counter; ?>">
					...
					<?php 
						...
						//Do recursion
						$counter = prime_custom_navigation_default_sub_items($parent_id, $counter, $parent_id, $type, 'menu'); 
			...
			elseif ($output_type == 'default')
			{
				...
				<li>
					...
					<?php 
						...
						//Do recursion
						$counter = prime_custom_navigation_default_sub_items($itemid, $counter, $parent_id, $type, 'default');
...
class prime_NavWidget extends WP_Widget {
	...
	function prime_NavWidget() {
		$widget_ops = array('description' => 'Use this widget to add one of your prime Custom Navigation Menus as a widget.' );
		parent::WP_Widget(false, __('prime - Custom Nav Menu', 'primethemes'),$widget_ops);      
	}
	...
	function widget($args, $instance) {  
		...
		{
			$table_name_menus = $wpdb->prefix . "prime_custom_nav_menus";
			$prime_result = $wpdb->get_results("SELECT menu_name FROM ".$table_name_menus." WHERE id='".$navmenu."'");
			$prime_custom_nav_menu_name = $prime_result[0]->menu_name;
			$menuexists = true;
		}
		//Do nothing
		else 
        		...
        		?>
						...
						<?php
							...
							if (get_option('prime_custom_nav_menu') == 'true') {
        						prime_custom_navigation_output('name='.$prime_custom_nav_menu_name.'&desc='.$navwidgetdescription);
        					}				
						?>
	...
	function form($instance) {        
		...
		if ($custom_menu_records > 0)
		{
			...
			?>
			 ...
			 <p>
	            <label for="<?php echo $this->get_field_id('navmenu'); ?>"><?php _e('Select Menu:','primethemes'); ?></label>
			...
			<p>
		        ...
		        <label for="<?php echo $this->get_field_id('navtitle'); ?>"><?php _e('Title:','primethemes'); ?></label>
			...
			<label for="<?php echo $this->get_field_id('navdeveloper'); ?>"><?php _e('Advanced Options:','primethemes'); ?></label><br />    	
			<span class="checkboxes">
		    ...
		    if ($checked == 'yes')
		    {
				...
				<label for="<?php echo $this->get_field_id('navdiv'); ?>"><?php _e('Wrap in container DIV:','primethemes'); ?></label><br />	
				<span class="checkboxes">
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('navul'); ?>"><?php _e('Wrap in container UL:','primethemes'); ?></label><br />    	
				<span class="checkboxes">
			...
			<p <?php if ($advanced_option_descriptions == 'no') { ?>style="display:none;"<?php } ?>>
	           ...
	           <?php
				...
				<label for="<?php echo $this->get_field_id('navwidgetdescription'); ?>"><?php _e('Show Top Level Descriptions:','primethemes'); ?></label><br />    	
				<span class="checkboxes">
		...
		else 
		{
			?>
			<p>
		    	<label><?php _e('The Custom Menu has not been configured correctly.  Please check your theme settings before adding this widget.','primethemes'); ?></label>
			</p>
			<?php
		}
	}
...
if (get_option('prime_custom_nav_menu') == 'true') 
{
	register_widget('prime_NavWidget');
}
