...
$new_width =  (int) abs (getRequest ('w', 0));
$new_height = (int) abs (getRequest ('h', 0));
$zoom_crop = (int) getRequest ('zc', 1);
$quality = (int) abs (getRequest ('q', 90));
$align = getRequest ('a', 'c');
$filters = getRequest ('f', '');
$sharpen = (bool) getRequest ('s', 0);
...
function function_three
 ($mime_type, $image_resized) {

    global $quality;

    $cache_file = function_nine ($mime_type);
...
function mime_type ($file) {
 ...
 ('no mime type specified');
    ...
    if (!preg_match ("/jpg|jpeg|gif|png/i", $mime_type)) {
		function_thirteen
 ('Invalid src mime type: ' . $mime_type);
    }
 ...
 * @param <type> $mime_type
 */
function function_seven ($mime_type) {

	if (CACHE_USE) {

		if (!function_eight ($mime_type)) {
 ...
 * @return <type> 
 */
function function_eight ($mime_type) {
 ...
 * @return string
 */
function function_nine ($mime_type) {
...
function function_eleven ($src) {
 ...
 ('error reading file ' . $src . ' from remote host: ' . curl_error ($ch));
					...
					if (!$img = file_get_contents ($src)) {
						function_thirteen
 ('remote file for ' . $src . ' can not be accessed. It is likely that the file permissions are restricted');
					}
 ...
 ('error writing temporary file');
				...
				if (!file_exists ($local_filepath)) {
					function_thirteen
 ('local file for ' . $src . ' can not be created');
				}
				...
				function_thirteen
 ('remote host "' . $url_info['host'] . '" not allowed');
...
function function_twelve ($src) {
 ...
 ('source is not a valid file');
	...
	if (filesize ($src) > MAX_FILE_SIZE) {
		function_thirteen
 ('source file is too big (filesize > MAX_FILE_SIZE)');
	}

	if (filesize ($src) <= 0) {
		function_thirteen
 ('source file <= 0 bytes. Possible external file download error (file is too large)');
	}
